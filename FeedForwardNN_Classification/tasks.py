...
def taskSquare(draw):
	...
	nn1 = nn.NeuralNetwork(2, 0.1, 50, 50)
...
def taskSemiCircle(draw):
	...
	# raise NotImplementedError
	nn1 = nn.NeuralNetwork(2, 0.25, 50, 30)
...
def taskMnist():
	...
	# raise NotImplementedError
	nn1 = nn.NeuralNetwork(10, 0.07, 10, 3)
	nn1.addLayer(FullyConnectedLayer(784, 15))
	nn1.addLayer(FullyConnectedLayer(15, 10))
	###############################################
	nn1.train(XTrain, YTrain, XVal, YVal, False, True)
	pred, acc = nn1.validate(XTest, YTest)
	print('Test Accuracy ', acc)
...
def taskCifar10():
	...
	nn1.addLayer(ConvolutionLayer([3, 32, 32], [10, 10], 4, 3))
	# nn1.addLayer(AvgPoolingLayer([6,6,6], [2,2], 2))
	nn1.addLayer(FlattenLayer())
	nn1.addLayer(FullyConnectedLayer(4*8*8, 30))
	...
	nn1.train(XTrain, YTrain, XVal, YVal, True, True,
	          loadModel=False, saveModel=True, modelName=modelName)
	pred, acc = nn1.validate(XTest, YTest)
	print('Test Accuracy ', acc)
